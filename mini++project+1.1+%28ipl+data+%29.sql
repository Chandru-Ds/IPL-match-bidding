...
Select count(MATCH_ID), ms.STADIUM_ID,s.STADIUM_NAME, s.CITY from 
ipl_match_schedule as ms join ipl_stadium as s on ms.STADIUM_ID = s.STADIUM_ID group by STADIUM_ID ;

-- 3.In a given stadium, what is the percentage of wins by a team which has won the toss?
Select ist.STADIUM_NAME, (sum((Case when Toss_winner=Match_winner then 1 else 0 end)) /count(*) )*100
 as 'percentage of wins' from ipl_stadium ist join ipl_match_schedule ms on ist.stadium_id =ms.STADIUM_ID 
 join ipl_match im on ms.match_id= im.match_id group by ist.STADIUM_NAME order by 1;
-- 4.Show the total bids along with the bid team and team name.
select BID_TEAM,Team_name,count(bidder_id) as bid_count 
from ipl_bidding_details bd join ipl_team t on bd.BID_TEAM=t.team_id 
group by bid_team; 

-- 5.Show the team id who won the match as per the win details.
select win_details, match_winner from ipl_match group by MATCH_WINNER,win_details;
...
sum(MATCHES_PLAYED) as total_matches_played, sum(MATCHES_WON) as total_matches_won,
sum(MATCHES_LOST) as total_matches_lost 
from ipl_team_standings as its join ipl_team as it
on its.team_id = it.team_id group by team_ID;
 
-- 7.Display the bowlers for the Mumbai Indians team.
SELECT player_name
 FROM ipl_team_players i1 JOIN ipl_team i2 
...
SELECT COUNT(*) AS 'count of All Rounders', i1.team_id, i2.team_name
 FROM ipl_team_players i1 JOIN ipl_team i2
 ON i1.team_id = i2.team_id
 JOIN ipl_player i3
 ON i1.player_id = i3.player_id 
 WHERE i1.player_role = 'All-Rounder' 
 GROUP BY team_id HAVING COUNT(*) > 4 ORDER BY COUNT(*) DESC;
...
join ipl_bidding_details bdg
on schd.SCHEDULE_ID =bdg.SCHEDULE_ID
join ipl_bidder_details bdr
on bdg.BIDDER_ID=bdr.BIDDER_ID
join ipl_bidder_points pts
...
from ipl_bidder_details bdr  join ipl_bidder_points pts
on bdr.BIDDER_ID=pts.BIDDER_ID 
join ipl_bidding_details bdg
...
where Ranks<4
union all
(select * from(select pts.BIDDER_ID,pts.TOTAL_POINTS,
rank() over(order by pts.TOTAL_POINTS ) as Ranks2,bdr.BIDDER_NAME,'Lowest_3_Bidders'
from  ipl_bidder_points pts join ipl_bidder_details bdr
...
insert into student_details values ('jayaram',01,'jayaram2021@gmail.com',097654321),('Nandhas',02,'nadhas2023@gmail.com',123456789), 
('mukil',03,'mukil2021@gmail.com',1246790742);
 
 insert into student_details_backup values('jayaram',01,'jayaram2021@gmail.com',097654321),('Nandhas',02,'nadhas@gmail.com',123456789), 
('mukil',03,'mukil2021@gmail.com',1246790742);
